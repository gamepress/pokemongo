export function calcData($http, $location, gameData) {
   ...
   function calculateLeague(
      ...
      if (level > max_lvl) {
         own_results["own_unchanged"] =
            "Your pokemon is over the maximum level.";
      } else {
         ...
         const own_stat_prod_sta = Math.floor(
            (pokemon.sta + sta) * own_curr_cpm,
         );
         const own_cp =
            (own_curr_cpm *
               own_curr_cpm *
         ...
         if (own_cp > max_cp) {
            own_results["own_unchanged"] =
               "Your pokemon is over the maximum CP.";
         } else {
            own_final_stat_prod =
               (own_stat_prod_atk * own_stat_prod_def * own_stat_prod_sta) /
               1000;
         }
      }
            ...
            for (var atk_iv = 0; atk_iv <= 15; atk_iv++) {
               ...
               for (var def_iv = 0; def_iv <= 15; def_iv++) {
                  ...
                  for (var sta_iv = 0; sta_iv <= 15; sta_iv++) {
                     ...
                     if (
                        final_stat_prod > final_results[max_cp].highest.value
                     ) {
   ...
   function calculateRankFromStatProd(
      own_final_stat_prod,
      final_results,
      sorted1500,
      max_cp,
   ...
   function calculateBerry(
      ...
      for (var i = 0; i < 25; i++) {
         ...
         var e9 =
            (1 - e8) * (1 - Math.pow(1 - catchrates_e9, curr_silvers_value));
         calc_array.p_silver.push(e9);

         var catchrates_e10 = 1 - Math.pow(f16, 2.5);
         var e10 =
            (1 - e9 - e8) *
            (1 - Math.pow(1 - catchrates_e10, curr_goldens_value));
   ...
   function getWeather(type, types) {
      if (!types) {
         return 1;
      }
   ...
   function calculateSingleCP(
      ...
      if (targetEvolution != null && evolve) {
         ...
         evoCost = gameData.calculateEvolutionCosts(
            pokemonName,
            targetEvolution,
         );
      }
   ...
   function calculateSingleCatchRate(
      pokemonName,
      level,
      ball,
      curve,
   ...
   //lvl 20
   //cpm = 0.5974001
   function calculateRaidIVs(pokemonName, pokemon, cp, weather) {
      service.results = {
         name: pokemonName,
         data: pokemon,
         cp: cp,
                     ...
                     if (CP == cp) {
                        ...
                        if (accept) {
                           ...
                           service.results.avgIV =
                              (service.results.minIV + service.results.maxIV) /
                              2;
                           service.results.stats.push(result);
                        }
                     }
   ...
   function calculateBubble(
      ...
      if (calcIVs) {
         ...
         if (defenderCPs.length == 0) {
            noIVsMessage =
               "No possible IV spreads found for the Pokemon entered.";
         }
      } else {
