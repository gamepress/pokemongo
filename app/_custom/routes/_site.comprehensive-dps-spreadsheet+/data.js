...
var pokemonDataFullURL = "/sites/default/files/pokemon-data-full-en-PoGO.json";
var moveDataFullURL = "/sites/default/files/move-data-full-PoGO.json";
var raidBossListURL = "/sites/default/files/raid-boss-list-PoGO.json";
...
function getPokemonIcon(dex) {
	...
	while (dex.length < 3)
		dex = '0' + dex;
	return "/themes/gamepressbase/assets/img/sprites/" + dex + "MS.png";
}
...
function getTypeIcon(type) {
	return "/sites/defauult/files/icon_" + (type || "none").toLowerCase() + ".png";
}
...
function fetchLevelSettings(oncomplete) {
	...
	$.ajax({
		url: "/sites/default/files/cpm.json",
		dataType: 'json',
		success: function (data) {
			Data.LevelSettings = [];
			for (var i = 0; i < data.length; i++) {
...
function fetchPokemonForms(oncomplete) {
	...
	$.ajax({
		url: "/sites/default/files/pogo-jsons/pogo_data_projection_10.json" + "?" + curTime,
		dataType: 'json',
		success: function (data) {
			Data.PokemonForms = [];
			for (let pkm of data) {
